<?php
/**
 * Holds the Unassigned Comments Page which shows only the Unassigned Comments
 *
 * @since		{{VERSION}}
 *
...
final class PYIS_Comment_Assignment_Unassigned {
	
	/**
	 * PYIS_Comment_Assignment_Unassigned constructor.
	 * 
	 * @since		{{VERSION}}
	 */
	...
	public function alter_screen() {
		...
		if ( $current_screen->id == 'comments_page_unassigned' ) {
		
			$current_screen->id = 'unassigned';
	...
	public function add_submenu_page() {
		...
		add_submenu_page(
			'edit-comments.php',
			__( 'Unassigned', 'pyis-comment-assignment' ),
			__( 'Unassigned', 'pyis-comment-assignment' ),
	...
	public function comment_status_links( $status_links ) {
		...
		if ( ! is_admin() || 
		   $current_screen->id !== 'unassigned' ) return $status_links;
		
		foreach ( $status_links as &$status_link ) {
			
			$status_link = str_replace( "edit-comments.php?", "edit-comments.php?page=unassigned&", $status_link );
	...
	public function submenu_page() {
		...
		// Change Title to "Unassigned Comments", regardless of Langugage setting
		$edit_comments = preg_replace( '#<h1([^>]*)>(.*?)' . __( 'Comments' ) . '(.*?)<\/h1>#is', '<h1$1>$2' . __( 'Unassigned Comments', 'pyis-comment-assignment' ) . '$3</h1>', $edit_comments );
		
		// Add a hidden Input with our Page Slug so that any Searching or other Form Interactions always return us to our page
		$edit_comments = preg_replace( '#<form id="comments-form" method="get">#is', '$0<input type="hidden" name="page" value="unassigned" /><input type="hidden" name="comment_status" value="unassigned" />', $edit_comments );
		...
		/**
		 * Allow modification of the Unassigned Comments page
		 * 
		 * @since		{{VERSION}}
		 * @return		string HTML
		 */
		$edit_comments = apply_filters( 'pyis_comment_assignment_unassigned_html', $edit_comments );
	...
	/**
	 * Restrict the Unassigned Comments Page to only querying Unassigned comments
	 * 
	...
	public function pre_get_comments( $query ) {
		...
		if ( $current_screen->id !== 'unassigned' ) return;
	...
	/**
	 * Filter wp_count_comments() on Assigned Comments pages so that it only counts for Unassigned Comments
	...
	public function wp_count_comments( $comment_stats = array(), $post_id = 0 ) {
		...
		if ( $current_screen->id !== 'unassigned' ) return $comment_stats;
		
		$count = wp_cache_get( "comments-unassigned-{$post_id}", 'counts' );
		if ( false !== $count ) {
			return $count;
		}
		...
		wp_cache_set( "comments-unassigned-{$post_id}", $stats_object, 'counts' );
	...
	private function get_comment_count( $post_id = 0, $user_id = false ) {
		...
		$where = $wpdb->prepare( "WHERE ({$wpdb->commentmeta}.meta_key = 'assigned_to' AND {$wpdb->commentmeta}.meta_value = '') OR ({$wpdb->commentmeta}.comment_id IS NULL)", $user_id );
		if ( $post_id > 0 ) {
...
$instance = new PYIS_Comment_Assignment_Unassigned();